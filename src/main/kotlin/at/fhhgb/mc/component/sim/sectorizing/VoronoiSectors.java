...
import at.fhhgb.mc.component.sim.sectorizing.voronoi.GrahamScan;
import at.fhhgb.mc.component.sim.sectorizing.voronoi.GraphEdge;
import at.fhhgb.mc.component.sim.sectorizing.voronoi.Voronoi;
...
* ID: 1510455019
*/
public class VoronoiSectors implements ISectorizing {
    ...
    public void createSectors(FloorManager floorManager, Graphics g, Matrix m_tMatrix) {
        ...
        for (int i = 0; i < floorManager.getFloor().getAccessPointList().size(); i++) {
            PixelPosition pixelPosition = floorManager.calculatePixelPositionFromMeter(floor.getAccessPoint(i).getPosition().getX(), floor.getAccessPoint(i).getPosition().getY());
            xvalues[i] = pixelPosition.getXPosition();
            yvalues[i] = pixelPosition.getYPosition();
        }
        ...
        for (List<GraphEdge> list : edgesByPolygon.values()) {
            ...
            if (list.size() < 3) {
                for (GraphEdge graphEdge : list) {
                    DrawingContext.drawLine((int) graphEdge.x1, (int) graphEdge.y1, (int) graphEdge.x2, (int) graphEdge.y2, g, m_tMatrix);
                }
                continue;
            }
        ...
        for (Polygon polygon : polygons) {
            if (position != null && polygon.contains(new Point(position.getX(), position.getY()))) {
                DrawingContext.drawVoronoi(polygon, g, m_tMatrix, true);
            } else {
                DrawingContext.drawVoronoi(polygon, g, m_tMatrix, false);
            }
        }
        ...
        DrawingContext.drawCells(bounds, g, m_tMatrix, false);
    ...
    /*
    @Override
    public void createSectors(FloorConverter floorConverter, Graphics g, Matrix m_tMatrix) {
        int n = 0;
